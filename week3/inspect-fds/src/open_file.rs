...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fdlink = fs::read_link(path).ok()?;
        let name = OpenFile::path_to_name(fdlink.to_str()?);

        let path = format!("/proc/{}/fdinfo/{}", pid, fd);
        let fdinfo = fs::read_to_string(path).ok()?;
        let curser = OpenFile::parse_cursor(&fdinfo)?;
        let access_mode = OpenFile::parse_access_mode(&fdinfo)?;

        Some(OpenFile::new(name, curser, access_mode))
    }
